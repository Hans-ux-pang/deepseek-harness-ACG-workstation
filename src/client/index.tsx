...
import { splitStore, SplitWorkspace, setSplitT, setSplitEnv, FxOverlay, getFxConf, subscribeFxConf, STOCK_WATCH_ID, STOCK_WATCH_ICON, type LayoutSpec, type SplitPane, type ConsoleCardData } from './split'
import { DhFloat, dhPrefs } from './digitalhuman'
...
type ViewState = {
  ...
  /** 控制室背景：off/plain/glow/photo（默认 glow）—— off = 关掉控制室自己的背景（窗格透明）*/
  consoleBg?: 'off' | 'plain' | 'glow' | 'photo'
  /** 控制室自身背景透明度（0-100，100 = 不透明，0 = 全透明露出后面的全屏壁纸；默认 100）*/
  consoleBgAlpha?: number
  /** 「看板娘」Live2D 挂件开关（默认关；关闭时不发起任何外部请求）*/
  consolePetOn?: boolean
  /** 看板娘模型索引（PET_MODELS 下标，默认 0）*/
  consolePetModel?: number
  /** 看板娘相对控制室右下角的内边距（px，默认 {x:14,y:14}）*/
  consolePetPos?: { x: number; y: number }
...
function loadView(): ViewState {
  try {
    ...
    return {
      ...
      consoleBg: p.consoleBg === 'off' || p.consoleBg === 'plain' || p.consoleBg === 'photo' ? p.consoleBg : 'glow',
      consoleBgAlpha: typeof p.consoleBgAlpha === 'number' && p.consoleBgAlpha >= 0 && p.consoleBgAlpha <= 100 ? Math.round(p.consoleBgAlpha) : undefined,
...
/** 「股票检测」种子标记：只种一次；用户之后删除/隐藏不会被重新种回 */
const STOCK_SEED_KEY = 'dsh.worktable.stockSeeded.v1'
/** 股票检测任务起始文案（预填「股票检测」项目自定义窗的需求框；会话建立后即它的任务书） */
const STOCK_STARTER = [
  '📈 股票检测任务书',
  '1. 目标：作为我的 A 股（沪深）个股监控助手，盯住自选股，异常时主动提醒。',
  '2. 自选股清单：读 localStorage dsh.worktable.stock.v1（JSON：{ codes: [\"600519.SH\", \"000001.SZ\", ...], thresholdPct: 3 }）。没有就先用网络查询并告诉我格式，让我添加。',
  '3. 每次会话开始，尽量先用联网工具拉一遍自选股实时行情；涨跌幅超过阈值（默认 ±3%，可让我改）或个股有大事件时，用一句话点出并给建议。',
  '4. 数据以你实际查到的为准，禁止编造价格/涨跌幅；查不到就如实说查不到。',
  '5. 需要沉淀的报告/表格/图表，写进本项目文件夹（若未设置，请提醒我在卡片绑定面板里设置项目文件夹）。',
].join('\n')
/** 种子：首次运行时向项目列表追加「股票检测」layout 项目卡（预置自定义窗，预填任务书文案） */
function ensureStockSeeded(): void {
  try {
    if (localStorage.getItem(STOCK_SEED_KEY) === '1') return
    const raw = localStorage.getItem(PROJECTS_KEY)
    const p = raw ? JSON.parse(raw) : { ...DEFAULT_PROJECTS }
    const layouts = Array.isArray(p.layouts) ? p.layouts : []
    if (!layouts.some((l: any) => l?.id === STOCK_WATCH_ID)) {
      layouts.push({
        id: STOCK_WATCH_ID,
        title: '股票检测',
        icon: STOCK_WATCH_ICON,
        top: null,
        main: [{
          id: 'stock-main',
          title: '股票检测',
          min: 260,
          tabs: [{ id: 'stock-tab1', title: '股票检测', content: { kind: 'builtin', type: 'custom' } }],
          active: 0,
        }],
      ...
      p.layouts = layouts
      p.order = [...(Array.isArray(p.order) ? p.order : []).filter((x: unknown) => x !== STOCK_WATCH_ID), STOCK_WATCH_ID]
      localStorage.setItem(PROJECTS_KEY, JSON.stringify(p))
    }
    localStorage.setItem(STOCK_SEED_KEY, '1')
  } catch { /* 种子失败不阻塞主流程 */ }
}

/** 待办清单 内建入驻项目（layout 项目卡，预置自定义窗；产出挂到其窗口） */
const TODO_WATCH_ID = 'wt-todo'
const TODO_WATCH_ICON = '✅'
const TODO_SEED_KEY = 'dsh.worktable.todoSeeded.v1'
const TODO_STARTER = [
  '✅ 待办清单任务书',
  '1. 目标：作为我的待办管理助手，维护一张随取随用的待办清单看板。',
  '2. 数据：读 localStorage dsh.worktable.todo.v1（JSON：{ items: [{ id, text, done }], nextId }）。清单为空就先问我要做什么再建。',
  '3. 操作：我可以说「加…」「完成…」「删除…」「列出待办」，你实时读写同一份数据。',
  '4. 产出：交互式待办看板做成单文件 HTML（列表 + 勾选 + 删除 + 新增输入，样式引用 /api/worktable/template/dshell.css），放入本项目文件夹；完成后写 widget-result.json 挂到「待办清单」窗口。',
  '5. 数据以 localStorage 为准，不要另建一套状态；查不到就如实说明。',
].join('\n')
/** 种子：首次运行向项目列表追加「待办清单」layout 项目卡（一次，之后删除/隐藏不再复活） */
function ensureTodoSeeded(): void {
  try {
    if (localStorage.getItem(TODO_SEED_KEY) === '1') return
    const raw = localStorage.getItem(PROJECTS_KEY)
    const p = raw ? JSON.parse(raw) : { ...DEFAULT_PROJECTS }
    const layouts = Array.isArray(p.layouts) ? p.layouts : []
    if (!layouts.some((l: any) => l?.id === TODO_WATCH_ID)) {
      layouts.push({
        id: TODO_WATCH_ID,
        title: '待办清单',
        icon: TODO_WATCH_ICON,
        top: null,
        main: [{
          id: 'todo-main',
          title: '待办清单',
          min: 260,
          tabs: [{ id: 'todo-tab1', title: '待办清单', content: { kind: 'builtin', type: 'custom' } }],
          active: 0,
        }],
        chatWidth: { default: 300, min: 260, max: 620 },
        topHeight: { default: 200, min: 120, max: 480 },
        chatSide: 'right',
      } satisfies LayoutSpec)
      p.layouts = layouts
      p.order = [...(Array.isArray(p.order) ? p.order : []).filter((x: unknown) => x !== TODO_WATCH_ID), TODO_WATCH_ID]
      localStorage.setItem(PROJECTS_KEY, JSON.stringify(p))
    }
    localStorage.setItem(TODO_SEED_KEY, '1')
  } catch { /* 种子失败不阻塞主流程 */ }
}

/** 地球旅行 内建项目（layout 项目卡，预置 globe 窗格：3D 地球 + 点一下就出发） */
const GLOBE_WATCH_ID = 'wt-globe'
const GLOBE_WATCH_ICON = '🌍'
/**
 * 独立种子标记（不复用 STOCK/TODO 的键）：那两个键在既有用户上已经是 '1'，
 * 复用会导致地球项目永远种不出来。换个键可保证既有用户也会被种一次。
 */
const GLOBE_SEED_KEY = 'dsh.worktable.globeSeeded.v1'
/** 种子：首次运行向项目列表追加「地球旅行」layout 项目卡（一次，之后删除/隐藏不再复活） */
function ensureGlobeSeeded(): void {
  try {
    if (localStorage.getItem(GLOBE_SEED_KEY) === '1') return
    const raw = localStorage.getItem(PROJECTS_KEY)
    const p = raw ? JSON.parse(raw) : { ...DEFAULT_PROJECTS }
    const layouts = Array.isArray(p.layouts) ? p.layouts : []
    if (!layouts.some((l: any) => l?.id === GLOBE_WATCH_ID)) {
      layouts.push({
        id: GLOBE_WATCH_ID,
        title: '地球旅行',
        icon: GLOBE_WATCH_ICON,
        top: null,
        main: [{
          id: 'globe-main',
          title: '地球旅行',
          min: 320,
          tabs: [{ id: 'globe-tab1', title: '地球旅行', content: { kind: 'builtin', type: 'globe' } }],
          active: 0,
        }],
        /* 对话窗给窄一点，把宽度让给地球 */
        chatWidth: { default: 300, min: 260, max: 620 },
      ...
      p.layouts = layouts
      p.order = [...(Array.isArray(p.order) ? p.order : []).filter((x: unknown) => x !== GLOBE_WATCH_ID), GLOBE_WATCH_ID]
      localStorage.setItem(PROJECTS_KEY, JSON.stringify(p))
    }
    localStorage.setItem(GLOBE_SEED_KEY, '1')
  } catch { /* 种子失败不阻塞主流程 */ }
}

/**
 * 游戏开发师 内建项目（layout 项目卡，预置 gamedev 窗格：爆款小游戏配方 + 一键复制 AI 提示词）
 *
 * 同样用独立种子键：GLOBE/TODO/STOCK 的键在既有用户上已经是 '1'，
 * 复用会让新项目永远种不出来。
 */
const GAMEDEV_WATCH_ID = 'wt-gamedev'
const GAMEDEV_WATCH_ICON = '🎮'
const GAMEDEV_SEED_KEY = 'dsh.worktable.gamedevSeeded.v1'
/** 种子：首次运行向项目列表追加「游戏开发师」layout 项目卡（一次，之后删除/隐藏不再复活） */
function ensureGameDevSeeded(): void {
  try {
    if (localStorage.getItem(GAMEDEV_SEED_KEY) === '1') return
    const raw = localStorage.getItem(PROJECTS_KEY)
    const p = raw ? JSON.parse(raw) : { ...DEFAULT_PROJECTS }
    const layouts = Array.isArray(p.layouts) ? p.layouts : []
    if (!layouts.some((l: any) => l?.id === GAMEDEV_WATCH_ID)) {
      layouts.push({
        id: GAMEDEV_WATCH_ID,
        title: '游戏开发师',
        icon: GAMEDEV_WATCH_ICON,
        top: null,
        main: [{
          id: 'gamedev-main',
          title: '游戏开发师',
          min: 360,
          tabs: [{ id: 'gamedev-tab1', title: '爆款小游戏', content: { kind: 'builtin', type: 'gamedev' } }],
          active: 0,
        }],
        /* 配方卡是双栏网格，对话窗给窄一点 */
        chatWidth: { default: 300, min: 260, max: 620 },
      ...
      p.layouts = layouts
      p.order = [...(Array.isArray(p.order) ? p.order : []).filter((x: unknown) => x !== GAMEDEV_WATCH_ID), GAMEDEV_WATCH_ID]
      localStorage.setItem(PROJECTS_KEY, JSON.stringify(p))
    }
    localStorage.setItem(GAMEDEV_SEED_KEY, '1')
  } catch { /* 种子失败不阻塞主流程 */ }
}

/** 控制室布局是否健全：主窗格首格存在 console 类型标签（旧版关标签的坏存档没有它） */function specHasConsoleTab(spec: LayoutSpec | undefined): boolean {
...
function buildConsoleSpec(t: (key: string) => string): LayoutSpec {
  return {
    ...
    chatWidth: { default: 280, min: 240, max: 620 },
    topHeight: { default: 200, min: 120, max: 480 },
    chatSide: 'right',
  }
}
...
/** 「数字人」内建项目：不预置卡片，由「添加项目」面板的开关打开（首次打开才落项目卡） */
const DH_ID = 'wt-digitalhuman'
const DH_ICON = '💃'
/** 数字人项目布局：单窗格（digitalhuman：形象 + 待机/思考/说话 + 朗读壳）+ 右侧对话 */
function buildDhSpec(name: string): LayoutSpec {
  return {
    id: DH_ID,
    title: name,
    icon: DH_ICON,
    ...
    main: [{
      ...
      title: name,
      min: 320,
      tabs: [{ id: 'dh-tab1', title: name, content: { kind: 'builtin', type: 'digitalhuman' } }],
      active: 0,
    }],
    chatWidth: { default: 340, min: 280, max: 620 },
    topHeight: { default: 200, min: 120, max: 480 },
    chatSide: 'right',
  }
}
...
/**
 * 一次性清理：早期版本往媒体库预置的两张示例图（极光 SVG + 波浪 JPEG）。
 * 用户明确不要它们，新装也不再预置。按**字节精确比对**删除，绝不误伤用户自己的素材。
 */
async function pruneSeededMedia(): Promise<{ removed: number; ids: string[] }> {
  const KEY = 'dsh.worktable.defaultBgPruned.v1'
  const empty = { removed: 0, ids: [] as string[] }
  try {
    if (localStorage.getItem(KEY) === '1') return empty
    const refs: Blob[] = [
      new Blob([DEFAULT_BG_SVG], { type: 'image/svg+xml' }),
      b64ToBlob(WAVE_BG_B64, 'image/jpeg'),
    ]
    const sameBytes = async (a: Blob, b: Blob): Promise<boolean> => {
    ...
    const ids: string[] = []
    for (const rec of await photoStore.list().catch(() => [])) {
      for (const ref of refs) {
        if (await sameBytes(rec.blob, ref)) { ids.push(rec.id); break }
      }
    }
    for (const id of ids) await photoStore.remove(id).catch(() => {})
...
/** 从任意块数组里取文本（跳过思考/工具块）——同步镜像与 history 两条通道共用 */
function blocksText(blocks: any): string {
  ...
  for (const b of blocks) {
    ...
    const s = typeof b.text === 'string' ? b.text : ''
    if (!s.trim()) continue
    if (kind === 'reasoning' || kind === 'thinking') continue
    if (kind === 'tool-call' || kind === 'tool_use' || kind === 'tool-result') continue
    out.push(s)
  }
  return out.join('\n')
}
...
/**
 * 最近一条助手回复的**完整**文本（数字人朗读用；与 lastTextOf 的区别：不洗代码、不截 220 字）。
 * 字段名容错：节点角色可能是 kind/type/role，块可能是 kind:'text' 或 type:'text'
 * ——先按 assistant 精确匹配，认不出角色则回落到最后一条带文本的节点（宁可念错也不静默）。
 */
function assistantSpeechOf(face: any): string {
  try {
    const partial = blocksText(face?.partial?.blocks)
    if (partial.trim()) return partial
    const nodes: any[] = Array.isArray(face?.nodes) ? face.nodes : []
    let fallback = ''
    for (let i = nodes.length - 1; i >= 0; i--) {
      const n = nodes[i]
      if (!n || typeof n !== 'object') continue
      const t = blocksText(n.blocks) || blocksText(n.content)
      if (!t.trim()) continue
      const role = n.kind ?? n.type ?? n.role
      if (role === 'assistant' || role === 'assistant-message' || role === 'assistantMessage') return t
      if (role === 'user' || role === 'steering' || role === 'context' || role === 'system') continue
...
/**
 * 第二条取文通道：宿主 history（异步只读事件流）。内存镜像取不到时兜底，只认 assistant/message，
 * 避免把用户自己的话念回来。与控制室冷会话预览走的是同一个只读通道。
 */
async function dhHistorySpeech(sid: string): Promise<string> {
  try {
    const face = sessionBridge?.sessions?.binding?.(sid)?.session?.getSnapshot?.()
    if (!face || typeof face.history !== 'function') return ''
    const r = await face.history({ maxMessages: 6 })
    const evs = r?.result?.value?.events
    if (!Array.isArray(evs)) return ''
    for (let i = evs.length - 1; i >= 0; i--) {
      const ev = evs[i]?.event
      ...
      const d = ev.data ?? {}
      const m = d.message ?? d
      const t = blocksText(m.content ?? m.blocks)
      if (t.trim()) return t
    }
  } catch {}
  return ''
...
/** 数字人环境：分栏窗格与悬浮小窗共用一份（成员都是模块级函数，无闭包陷阱） */
const dhEnvObject = {
  subscribe: (fn: () => void) => {
    sessionsSnapshotStore.listeners.add(fn)
    return () => { sessionsSnapshotStore.listeners.delete(fn) }
  },
  read: (sid: string) => dhRead(sid),
  readAsync: (sid: string) => dhHistorySpeech(sid),
  send: (text: string) => promptIntoSession(sessionScopeStore.snapshot?.sessionId ?? '', text),
}

/** 会话是否正在运行（宿主快照优先，会话面兜底——两者都可能滞后） */
function sessionRunningOf(sid: string): boolean {
  try {
    if (sessionsSnapshotStore.snapshot?.byId?.[sid]?.running === true) return true
    const face = sessionBridge?.sessions?.binding?.(sid)?.session?.getSnapshot?.()
    if (face?.running === true) return true
  } catch {}
  return false
}

/** 数字人窗格的数据面：某会话的「最近回复 + 运行态」（纯读内存镜像，零 Token） */
function dhRead(sid: string): { text: string; running: boolean } {
  if (!sid) return { text: '', running: false }
  let text = ''
  try {
    const face = sessionBridge?.sessions?.binding?.(sid)?.session?.getSnapshot?.()
    text = assistantSpeechOf(face)
  } catch {}
  return { text, running: sessionRunningOf(sid) }
}
...
function WorktableSection(props: any) {
  ...
  /** 数字人悬浮小窗：默认关（不动用户界面），点控制室「数字人」磁贴或小窗 ✕ 切换 */
  const [dhFloatOpen, setDhFloatOpen] = useState<boolean>(() => dhPrefs.read().floatOn === true)
  ...
  /** 数字人开关句柄：env.console 只建一次（[] 依赖），经 ref 取当前渲染的实现，避免旧闭包 */
  const dhToggleRef = useRef<(() => void) | null>(null)
  ...
  useEffect(() => {
    const env = {
      ...
      custom: {
        ...
        getProjectStarter: (id: string) => (id === STOCK_WATCH_ID ? STOCK_STARTER : id === TODO_WATCH_ID ? TODO_STARTER : ''),
      ...
      console: {
        ...
        // 宿主当前活跃会话是否正在运行（忙碌特效的兜底触发源；未绑定到卡片也能点亮）
        getScopeBusy: () => {
          try {
            const snap = sessionsSnapshotStore.snapshot
            const cur = snap?.current ?? ''
            if (!cur) return false
            const e = snap?.byId?.[cur]
            return !!e && e.running === true
          } catch { return false }
        },
        // 「操控电脑」：指令发到控制室绑定会话，由其 AI 执行（系统操作仍需宿主授权）
        sendRemote: async (text: string) => {
          const sid = projectsRef.current.projects.bindings[CONSOLE_ID]
          if (!sid || !String(text).trim()) return false
          try { await promptIntoSession(sid, String(text).trim()); return true } catch { return false }
        },
        ...
        setBg: (m: 'off' | 'plain' | 'glow' | 'photo') => persistView({ consoleBg: m }),
        getBgAlpha: () => viewRef.current.consoleBgAlpha ?? 25,
        setBgAlpha: (n: number) => persistView({ consoleBgAlpha: n }),
        getPhotoLib: async () => {
        ...
        getPetOn: () => viewRef.current.consolePetOn === true,
        setPetOn: (v: boolean) => persistView({ consolePetOn: v }),
        getPetModel: () => viewRef.current.consolePetModel ?? 0,
        setPetModel: (v: number) => persistView({ consolePetModel: v }),
        getPetPos: () => viewRef.current.consolePetPos ?? { x: 14, y: 14 },
        setPetPos: (v: { x: number; y: number }) => persistView({ consolePetPos: { ...v } }),
        ...
        onAdd: () => openAddPanel(),
        // 数字人打开开关（控制室网格里「添加项目」旁那张磁贴）
        toggleDh: () => dhToggleRef.current?.(),
        onOpen: (id) => {
      ...
      // 数字人窗格：会话快照订阅 + 最近回复读取（纯读镜像，零 Token；不碰宿主状态）
      dh: {
        subscribe: (fn: () => void) => {
          sessionsSnapshotStore.listeners.add(fn)
          return () => { sessionsSnapshotStore.listeners.delete(fn) }
        ...
        read: (sid: string) => dhRead(sid),
        // 第二条取文通道（内存镜像为空时兜底；只认 assistant/message）
        readAsync: (sid: string) => dhHistorySpeech(sid),
        // 语音聊天：识别出的一句话送进当前会话（走既有 queue 投递路径，与「发送到会话」同源）
        send: (text: string) => promptIntoSession(sessionScopeStore.snapshot?.sessionId ?? '', text),
      },
    }
    setSplitEnv(env)
  ...
  // 媒体库一次性清理：删掉早期预置的两张示例图；若控制室照片背景正指向它们 → 清指针并退回流光
  useEffect(() => {
    void (async () => {
      const { removed, ids } = await pruneSeededMedia()
      if (removed === 0) return
      const active = viewRef.current.consoleBgPhotoId ?? ''
      if (active && ids.includes(active)) {
        persistView({ consoleBgPhotoId: '' })
        const rest = await photoStore.list().catch(() => [])
        if (rest.length === 0 && (viewRef.current.consoleBg ?? 'glow') === 'photo') persistView({ consoleBg: 'glow' })
      }
      notifyConsole()
    })()
  }, [])
...
function buildCustomLayoutPrompt(req: string): string {
  ...
  /** 数字人开关：未开 → 建/取项目卡并打开；已开 → 关闭（项目卡留在列表里）。
   *  入口 = 控制室网格里「添加项目」旁那张磁贴（env.console.toggleDh → dhToggleRef）。 */
  const toggleDigitalHuman = () => {
    invalidatePickState()
    const isOpen = !!splitStore.active && splitStore.spec?.id === DH_ID
    setAddOpen(false)
    if (isOpen) { splitStore.close(); return }
    const existing = projects.layouts.find((l) => l.id === DH_ID)
    let spec = (projects.views[DH_ID] ?? existing) as LayoutSpec | undefined
    if (!spec) {
      spec = buildDhSpec(t('dh.projectTitle'))
      const created = spec
      // 首次打开才落项目卡（不在启动时预置，避免给不想要的人多一张卡）
      persistProjects((prev) => (prev.layouts.some((l) => l.id === DH_ID) ? prev : {
        ...prev,
        layouts: [...prev.layouts, created],
        order: [...prev.order.filter((x) => x !== DH_ID), DH_ID],
      }))
    }
    openSplit(spec)
    reportUsed(DH_ID)
  }
  dhToggleRef.current = () => setDhFloatOpen((v) => { dhPrefs.write({ ...dhPrefs.read(), floatOn: !v }); return !v })
  ...
  const openConsole = useCallback((explicitBound?: string | null) => {
    ...
    let spec = broken ? buildConsoleSpec((k) => t(k as WorktableKey)) : (saved as LayoutSpec)
    // 比例迁移：已存聊天窗默认宽 > 280 时收窄，把更多面积让给控制室（仍可拖分隔条调回）
    if (!broken) {
      const cw: any = spec?.chatWidth
      if (cw && typeof cw.default === 'number' && cw.default > 280) {
        spec = { ...spec, chatWidth: { default: 280, min: 240, max: typeof cw.max === 'number' ? Math.max(cw.max, 320) : 620 } } as LayoutSpec
        persistProjects((prev) => ({ ...prev, views: { ...prev.views, [CONSOLE_ID]: spec } }))
      }
    }
  ...
  // 全局忙碌信号：宿主当前活跃会话 running，或任一控制室卡片 busy（驱动覆盖整个界面的特效层）
  const fxBusyNow = (() => {
    try {
      const snap = sessionsSnapshotStore.snapshot
      const cur = snap?.current ?? ''
      if (cur) { const e = snap?.byId?.[cur]; if (e && e.running === true) return true }
    } catch {}
    try { return getConsoleCards().some((c) => c.status === 'busy') } catch { return false }
  })()
  ...
  return (
    <div ref={rootRef} className={'dsh-wt_section' + (isFloat ? ' dsh-wt_float' : '')} style={isFloat ? floatStyle : dockedStyle}>
      ...
      {dhFloatOpen && (
        <DhFloat
          env={() => dhEnvObject}
          sessionId={() => sessionScopeStore.snapshot?.sessionId ?? ''}
          t={(k: string, p?: Record<string, string>) => t(k as WorktableKey, p)}
          onClose={() => { setDhFloatOpen((v) => { dhPrefs.write({ ...dhPrefs.read(), floatOn: !v }); return !v }) }}
        />
      )}
      {addOpen && <div className="dsh-wt_popBackdrop" onClick={() => { invalidatePickState(); setAddOpen(false) }} />}      {addOpen && (
...
export function apply(ctx: any) {
  // 首次运行：种子化「股票检测」「待办清单」「地球旅行」与「游戏开发师」项目卡（在首个组件挂载前执行）
  ensureStockSeeded()
  ensureTodoSeeded()
  ensureGlobeSeeded()
  ensureGameDevSeeded()
  ...
  // 玻璃 UI：把宿主 DSH 面板底色全部放开（styles.ts 里 html[data-wt-glass=on] 的 token 覆盖，
  // 覆盖全部底色 token：bg-base/layer-1/2/3、bg-overlay（弹层/菜单/对话框）、sidebar-fill、
  // 气泡/主输入框/登录输入/选择器/提示浮层）。
  // alpha 由 --wt-glassA 驱动 → 「UI 透明度」滑杆即时生效；插件卸载时移除标记，宿主恢复原样。
  // 同一处还落「全部静息动效」总开关的全局标记 data-wt-still（= on 时停掉纯 CSS 的待机动效：
  // 流光光晕 blob、玻璃板浮动/焦散；canvas 那几层由 React 挂载门控制，见 split.tsx）。
  ctx.effect(() => {
    const root = document.documentElement
    const applyGlass = () => {
      const c = getFxConf()
      try { root.setAttribute('data-wt-glass', c.glassOn ? 'on' : 'off') } catch {}
      try { root.setAttribute('data-wt-still', c.allIdle ? 'off' : 'on') } catch {}
      const a = Math.max(0.04, Math.min(0.96, 1 - c.glassAlpha / 100))
      try { root.style.setProperty('--wt-glassA', String(Math.round(a * 1000) / 1000)) } catch {}
    }
    applyGlass()
    const unsubscribe = subscribeFxConf(applyGlass)
    return () => {
      try { unsubscribe() } catch {}
      try {
        root.removeAttribute('data-wt-glass'); root.style.removeProperty('--wt-glassA')
        root.removeAttribute('data-wt-still')
      } catch {}
    }
  }, 'dsh-worktable: glass ui')
